...
### Visualizar en tabla
univ <- c('UCM', 'UAM', 'UAM', NA, 'UCM', 'UCM')
univf <- factor(univ, levels = c("UAM", "UCM", "UNED"))
table(univ)  # Devuelve la tabla de frecuencias, pero no tiene en cuenta los NA's porque es un vector.
table(univf) # ver diferencia con el anterior: devuelve los levels, lo que hace que se tenga en cuenta los NA's implícitamente (mete los NA's donde no hay valores)
#¿y el dato perdido?
univf2 <- addNA(univf)#añade un nivel al factor que contiene los datos perdidos (NA's)
univf2
table(univf2)

prueba <- addNA(genero);prueba#¿y si añadimos e nivel NA a un factor que no tiene valores perdidos?
table(genero)

### Operar a partir de los niveles de una variable categorica
edad <- c(21, 36, 19, 20, 20, 19)
...
rb <- rbind(v, ch); rb
class(rb)
typeof(rb) #ha cambiado el formato de v de números a tipo character
...
y <- c(10, 20)
z <- 100
x + y + z #como y no es de longitud proporcional a x, 
...
#de la matriz a las que aplicar la funcion. Margin se refiere a las filas (1) o columnas (2), que serán los elementos a los que se aplicará la función.
...
x <- 0:5; x
x < 3 #menor
which(x > 2) # Nos **devuelve los índices** de **los elementos de la variable** que cumplen esa condicion
which(x > 20)  # Obsérvese qué ocurre si no hay elementos que cumplan la condición.
x >= 3 #mayor o igual
y <- c(1, 1, 2, 4, 4, 4)
x != y #diferente de 
x == y #igual que
y[y < 3] #devuelve los ELEMENTOS que cumplen la condición (no como which que devuelve los índices de esos valores).
...
###FUNCION PASTE
v1<-c("A","B"); v2<-2:3
codigos <- paste(v1,v2); codigos#para unir vectores en formato character (crear codigos), por defecto los une con un espacio.
class(codigos)
codigos <- paste(v1,v2, sep = "");codigos #separar en varios elementos
codigos <- paste(v1,v2, sep = ".");codigos
codigos <- paste(v1,v2, collapse="");codigos #juntar en un solo elementos. Obsérvese cómo quedan los objetos que en otro caso no se unirían.
...
seq(along.with = c(3, 4, 5, 6, 7)) #crea una secuencia de longitud igual a la del vector del argumento along.with. Por defecto empieza desde el 1.
seq(from = 10, to = 13, # Aquí hacemos que no empiece en 1 (especificamos el intervalo).
    along.with = c(3, 4, 5, 6, 7))
seq(from = 10, to = 13, 
    length.out = 5) # length.out es como el anterior, pero señalando la longitud del vector directamente, sin tomar otro como base.
...
sort(n, decreasing=T) # No tiene en cuenta los NA's ni los NAN's.
rank(n)           #asigna un rango a los valores, yendo de los valores más pequeños a los más grandes;
#es util para tratar valores perdidos y valores repetidos
rank(n, na.last=F)
...
rank(r) #asigna un valor medio a cada valor repetido, esto es, si hay dos 1 les corresponde los rangos 1 y 2 por defecto, luego para ponerles el mismo, les pone el 1.5 (valor intermedio) a los dos.
rank (r, ties.method = "min") # Te redondea hacia abajo los rangos asignados en la línea anterior.
rank (r, ties.method = "max") # Redondea hacia arriba.
...
## Ejemplo de eliminación de filas con NA usando un filter:
...
?mutate
#permite añadir nuevas variables (util para mezclar bases, p.ej.)
