...
#Consejo: dar siempre nombre al gráfico y a las variables
hist(encuesta$Nota_PAU,
     xlab   = "Nota",
     ylab   = "Frecuencia",
...
#¿que variale de nuestra base de datos es categorica para poner un ejemplo de barplot?
        ...
        barplot(table(genero),
                xlab = "Género",
                ylab = "Frecuencia",
                main = "Número de hombres y mujeres",
                col = "dodgerblue",
                border = "darkorange")
        ...
        barplot(table(genero, encuesta$Edad), legend.text = levels(genero)) # Permite poner una leyenda especificando lo que queremos poner en la leyenda (en este caso coincide con 
                                                                            # lo que pondría por defecto, que es lo que vemos en la siguiente línea).
        barplot(table(genero,encuesta$Edad), beside = T, legend.text = T) # Mejor visualización usando besides = T, pone las barras una al lado de otra en vez de una encima de otra.
...
encuesta$Nota_PAU [encuesta$Nota_PAU >=8] <- NA   # Elimina valor extremo (los NA's no se representan gráficamente).
...
plot(encuesta$Nota_PAU , encuesta$Edad, data = encuesta, # obsérvese que al usar ~ las ordenadas son lo que está a la izquierda del símbolo y con "," pasan a ser las abcisas.
     xlab = "Notas PAU",
     ylab = "Edad",
