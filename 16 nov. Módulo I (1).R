...
ejemplo_entero <- 1L #si quiero crear un integer debo explicitarlo a travÃ©s de la letra L, sino es considerado numÃ©rico
ejemplo_numerico <- 3.14 #es importante saber que R al estar en inglÃ©s utiliza el punto para los decimales
...
#aparecer constantemente en los dataset si hay celdas vacÃ­as
is.na(ejemplo_NA)# ¿qué hace esta funciona?

#TIPOS DE OBJETOS

###MATRICES: vectores atomicos con dos dimensiones (filas y columnas)
m <- matrix(1:12, nrow = 3, ncol = 4); m #por defecto, coloca los nºs por columnas
...
ejemplo_factor <- factor("femenino", "masculino", "transgénero") #En el environment vemos Factor w/ 1 level
ejemplo_factor2 <- factor(c("femenino", "masculino", "transgénero"), labels = "género") 
...
###DATA.FRAMES: listas que permiten acceder a los datos de modo matricial
